...
names=$(cat /etc/urvcluster.conf | grep "HOSTS_NAME" | cut -d= -f2)
max_time=$(cat /etc/urvcluster.conf | grep "MAX_TIME" | cut -d= -f2)
scheduler=$(cat /etc/urvcluster.conf | grep "SCHEDULER" | cut -d= -f2)
